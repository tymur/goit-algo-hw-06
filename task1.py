import networkx as nx
import matplotlib.pyplot as plt

# === Створюємо граф центральних станцій Київського метрополітену ===
G = nx.Graph()

# Червона лінія (M1)
m1_stations = ["Університет", "Театральна", "Хрещатик", "Арсенальна"]
G.add_nodes_from(m1_stations, line='M1', color='red')
G.add_edges_from([
    ("Університет", "Театральна"),
    ("Театральна", "Хрещатик"),
    ("Хрещатик", "Арсенальна")
])

# Синя лінія (M2)
m2_stations = ["Контрактова площа", "Поштова площа", "Майдан Незалежності", "Льва Толстого", "Олімпійська"]
G.add_nodes_from(m2_stations, line='M2', color='blue')
G.add_edges_from([
    ("Контрактова площа", "Поштова площа"),
    ("Поштова площа", "Майдан Незалежності"),
    ("Майдан Незалежності", "Льва Толстого"),
    ("Льва Толстого", "Олімпійська")
])

# Зелена лінія (M3)
m3_stations = ["Лук’янівська", "Золоті ворота", "Палац спорту", "Кловська", "Печерська", "Дружби народів", "Видубичі"]
G.add_nodes_from(m3_stations, line='M3', color='green')
G.add_edges_from([
    ("Лук’янівська", "Золоті ворота"),
    ("Золоті ворота", "Палац спорту"),
    ("Палац спорту", "Кловська"),
    ("Кловська", "Печерська"),
    ("Печерська", "Дружби народів"),
    ("Дружби народів", "Видубичі")
])

# Пересадочні вузли
transfer_stations = [
    ("Театральна", "Золоті ворота"),          # M1 <-> M3
    ("Хрещатик", "Майдан Незалежності"),      # M1 <-> M2
    ("Льва Толстого", "Палац спорту")         # M2 <-> M3
]
G.add_edges_from(transfer_stations)

# === Фіксовані координати (приблизне географічне розташування) ===
pos = {
    # Червона лінія
    "Університет": (0, 2), "Театральна": (0, 1), "Хрещатик": (0, 0), "Арсенальна": (1, -0.5),
    # Синя лінія
    "Контрактова площа": (-2, 3), "Поштова площа": (-1, 2), "Майдан Незалежності": (-1, -0.5),
    "Льва Толстого": (-1, -1.5), "Олімпійська": (-1, -2.5),
    # Зелена лінія
    "Лук’янівська": (3, 3), "Золоті ворота": (2, 1), "Палац спорту": (2, -1), "Кловська": (3, -2),
    "Печерська": (4, -3), "Дружби народів": (5, -4), "Видубичі": (6, -5)
}

# === Візуалізація графа ===
plt.figure(figsize=(12, 8))

# Кольори вершин
node_colors = [G.nodes[n]['color'] for n in G.nodes()]
# Стиль ребер: пересадки підсвічуємо окремим кольором
# Стиль ребер: пересадки товстіші і оранжеві
edge_colors = []
edge_widths = []
for u, v in G.edges():
    if (u, v) in transfer_stations or (v, u) in transfer_stations:
        edge_colors.append("orange")  # пересадка
        edge_widths.append(3.5)       # товще для пересадки
    else:
        edge_colors.append("gray")    # звичайні ребра
        edge_widths.append(1.5)

# Малюємо граф
nx.draw(
    G,
    pos,
    with_labels=True,
    node_color=node_colors,
    edge_color=edge_colors,
    node_size=1500,
    font_size=10,
    font_weight="bold"
)

plt.title("🚇 Центральні станції Київського метрополітену з пересадками", fontsize=14)
plt.show()
